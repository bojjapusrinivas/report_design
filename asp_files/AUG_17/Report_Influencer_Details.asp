...
<div class="container floated">
	...
	<div class="row mb-0 mx-0">
		...
		<%Set RSInfpeople = Server.CreateObject("ADODB.Recordset")
			...
			inf_Query="SELECT sno as id, first_name as firstname,Last_name as lastname,filename,companyName,title as headline,'' as imgorder, (case when image_path is null then logo_local else image_path end) as image_url,Filename_MD5 ,Cmp_hidden_code as cmpid,market_acumen,EmployeeCountry as location,mcpcodes from GIA_MC_PROGRAM.dbo.vwInfluencersPeople WHERE sno=" & trim(request("id")) & " and filename='" & trim(request("code")) &"'"		
            ...
            <div  class="table-responsive border-0">
               <div class="mt-3 position-relative pb-3 d-flex flex-column">
                  ...
                  <table id="Influencer_table">
					...
					inf_ProjectQuery="SELECT  code,title,file_name,dop  from stat_globind.dbo.mcpnewtopic WHERE code in (select distinct trim(token) from dbo.String_Split('" & trim(RSInfpeople("mcpcodes")) &"',';'))"		
