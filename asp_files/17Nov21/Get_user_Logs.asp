    ...
    <div class="container"> 
		...
		IF Trim(request("encID"))<>"" THEN
					...
					<form name="frmGRP1" method="post" >
					 ...
					 '' response.write Source_Email & " Surce"
						...
						<a class='btn-gia-primary text-center  rounded-0 shadow p-1 mt-1  font-weight-bold mb-1 mx-1 '  name='btnsubmit' data-toggle='collapse' href='#resend_block' role='button' aria-expanded='false' aria-controls='collapseExample'>RE-SEND LOGIN</a> 
						<a class='btn-gia-primary text-center  rounded-0 shadow p-1 mt-1  font-weight-bold mb-1 mx-1'  name='btnsubmit' onclick="return Block_user('<%=Disp_Status%>','<%=trim(request("code"))%>');" ><%=Disp_Status%> USER LOGIN</a> 
						<a class='btn-gia-primary text-center  rounded-0 shadow p-1 mt-1  font-weight-bold mb-1 mx-1' name='btnsubmit'  href='GIA_Send_Collaborate.asp?code=<%=trim(Report_ID)%>&encID=<%=trim(request("encID"))%>&CHKemail=<%=User_Email%>'>UPDATE ASSIGNMENTS</a>
						<a class='btn-gia-primary text-center  rounded-0 shadow p-1 mt-1  font-weight-bold mb-1 mx-1'  name='btnsubmit' data-toggle='collapse' href='#send_message' role='button' aria-expanded='false' aria-controls='collapseExample'>SEND MESSAGE</a> 
						<a  class="btn-gia-primary text-center  rounded-0 shadow p-1 mt-1  font-weight-bold mb-1 mx-1" onclick="javascript:openwinMessages('<%=Trim(request("code"))%>','<%=Trim(request("encID"))%><%=get_col_email_string%>');" >VIEW MESSAGES</a>
	  ...
	  <div class="w-xl-85 w-lg-85 w-md-100 mt-2 mx-auto"  id="main2">
			...
			<div class="card card-body">
				<div >
						<div class="mb-3"><p class="font-weight-bold mb-1 ">To Email(s) :</p> 
							...
							<input type='text' id='example_emailSUI2' name='example_emailSUI' class='form-control border' value=''>

							<!--input type="text" class="w-100 mt-2" readonly placeholder="Email" id="muser" name="muser" value="" style="width:100% !important"--->
			...
			<div class="card card-body">
				<div >
						<div class="mb-3"><p class="font-weight-bold mb-1 ">To Email:</p> 
							...
							<input type='text' id='example_emailSUI2' name='example_emailSUI2' class='form-control border' value=''>
							<!--input type="text"  class="w-100 mt-2"  placeholder="Email" id="musermail" name="musermail" style="width:100% !important" value="<%=client%>"-->
		...
		<%if trim(request("btnRsendsubmit"))="RESEND" Then
			...
			sqlMsgInst="INSERT INTO [App_User_Collaborate_Messages] ([From_ID],[To_ID],[Message_Subject],[Message_Text],[Date_Sent],[Message_Status],[Message_Type]) VALUES  ('" & fromClient & "','" & client & "','" & replace(subject_line,"'","`") & "','" & replace(MessageBody,"'","`") & "',getdate(),'N','RESEND')" 
...
<script src="css/multiple-emails.js"></script>
...
<script type="text/javascript">
    ...
    $(function() {
        //To render the input device to multiple email input using SemanticUI icon
        $('#example_emailSUI2').multiple_emails({theme: "SemanticUI"});
