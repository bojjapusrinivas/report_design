...
function AddComp_metris(cpcode,mcpcode) {	 
 ...
 //alert(region);
	...
	//alert(vtxtRval);
	valsessiontxt = $("#txtsession"+vcpCode).val();
		...
		<div class="w-100  mb-2 ">
			<h3 class="text-primary text_orange font-weight-bold m-0 line-height-'normal mt-2 text-center mb-3" style="font-size:18px;">COMPETITIVE METRICS</h3><input type="hidden" value="<%=session("EID")%>" id="txtsession">
				...
				if trim(completed)="Y" then
					sql1 = "select top 10  company,filename, MCPCODES  FROM [GIA_MC_PROGRAM].[dbo].[InfluencersCompanies] WHERE filename in  (SELECT code FROM  stat_globind.dbo.MCP_CPCodes where mcpcode='" & trim(code) &"') order by company"
				else
