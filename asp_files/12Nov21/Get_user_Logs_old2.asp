		...
		IF Trim(request("encID"))<>"" THEN
					...
					<form name="frmGRP1" method="post" >
					  ...
					  Set Conn9 = Server.CreateObject("ADODB.Connection")
						...
						''response.write sql9
						response.write "<div class='w-xl-85 w-lg-85 w-md-100 mt-2 mx-auto'> <div class='w-100 font-weight-bold text-center' style='font-size:17px;'> LOGIN DETAILS</div>" 
					  
						rs9.open sql9, conn9
						if not rs9.eof then
						
						response.write "<div class='w-100'><table  class='table p-0 m-0' id='responsive_collaborate_table'> <thead class='thead-dark'><tr role='row'><th role='columnheader' style='text-align:center' scope='col'>SNO</th><th  scope='col' role='columnheader'>GIA_ID</th><th role='columnheader' scope='col' style='width:165px;'>EMAIL</th><th role='columnheader' style='width:230px;' scope='col'>LOGIN DATE</th><th role='columnheader' style='width:165px;' scope='col'>REPORT CODE</th><!--<td role='columnheader'>OPTIONS</td>--></tr></thead><tbody role='rowgroup'>"
						...
						gtVal = 1
							...
							Set rscheck1 = Server.CreateObject("ADODB.Recordset")
									...
									response.write "<tr role='row'> <td role='cell' style='text-align:center;' data-label='SNO'>"&gtVal&"</td><td  role='cell' data-label='GIA ID' >"&trim(rs9("userid"))&"</td><td  role='cell' data-label='EMAIL'>"&trim(rs9("emailid"))&"</td><td  role='cell' data-label='LOGIN DATE'>"&trim(rs9("login_date"))&"</td><td  role='cell' data-label='REPORT CODE'>"&trim(rs9("log_type"))&"</td>"
						...
						response.write "<div class='w-100 text-center font-weight-bold my-3 p-2' style='font-size:17px'>OTHER OPTIONS</div>"
						...
						<a class='button1 mx-auto rounded-0 shadow mx-2' style='width:150px;font-weight:bold;' name='btnsubmit' data-toggle='collapse' href='#resend_block' role='button' aria-expanded='false' aria-controls='collapseExample'>RE-SEND LOGIN</a> 
						 <a class='button1 mx-auto rounded-0 shadow mx-2' style='width:150px;font-weight:bold;' name='btnsubmit' onclick=return Block_user('<%=Disp_Status%>','<%=trim(request("code"))%>');" ><%=Disp_Status%> USER LOGIN</a> 
						  <a class='button1 mx-auto rounded-0 shadow mx-2' style='width:150px;font-weight:bold;' href='GIA_Send_Collaborate.asp?code=<%=trim(Report_ID)%>&encID=<%=trim(request("encID"))%>&CHKemail=<%=User_Email%>'>UPDATE ASSIGNMENTS</a>
