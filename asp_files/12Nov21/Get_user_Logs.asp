...
a.btn-gia-primary:hover, a.btn-gia-primary:focus {
        color: #fff;
}
  ...
  <div class="page-content"> 
    ...
    <div class="container"> 
      ...
      <div >
					...
					<form name="frmGRP1" method="post" >
					  ...
					  Set Conn9 = Server.CreateObject("ADODB.Connection")
						...
						MessageBody = MessageBody & "Hello "&trim(First_Name) & "  "&trim(Last_Name) & "," & vbcrlf & vbcrlf
							...
							MessageBody = MessageBody & "Email ID: " & trim(toemail) & "" & vbcrlf
						...
						<a class='btn-gia-primary text-center  rounded-0 shadow p-2 mt-2  font-weight-bold mb-1 '  name='btnsubmit' data-toggle='collapse' href='#resend_block' role='button' aria-expanded='false' aria-controls='collapseExample'>RE-SEND LOGIN</a> 
						 <a class='btn-gia-primary text-center  rounded-0 shadow mx-2 p-2 mt-2 font-weight-bold mb-1'  name='btnsubmit' onclick=return Block_user('<%=Disp_Status%>','<%=trim(request("code"))%>');" ><%=Disp_Status%> USER LOGIN</a> 
						  <a class='btn-gia-primary text-center  rounded-0 shadow  p-2 mt-2  font-weight-bold mb-1' name='btnsubmit'  href='GIA_Send_Collaborate.asp?code=<%=trim(Report_ID)%>&encID=<%=trim(request("encID"))%>&CHKemail=<%=User_Email%>'>UPDATE ASSIGNMENTS</a>
		...
		%>
	  <div class="w-xl-85 w-lg-85 w-md-100 mt-2 mx-auto">
		<div class="collapse" id="resend_block">
			<div class="card card-body">
				<div >
						<div class="mb-3"><p class="font-weight-bold mb-1 ">To Email(s) :</p> <input type="text" class="w-100 mt-2" placeholder="Subject" id="muser" name="muser" value="<%=client%>"></div>
						...
						<button type="submit" class="btn btn-gia-primary shadow rounded-0 mx-auto mt-2" name="btnsubmit" onclick="return resend_message();" value="INVITE"><small>SEND MESSAGE</small></button>
						</div>
						</div>
			</div>
		</div>
