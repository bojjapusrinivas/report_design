...
function AddComp_metris(cpcode,mcpcode) {	 
	...
	var rvcpCode = vcpCode.replace('-', '');
	
    var vtxtRank = $("#Rank"+rvcpCode).val();
			...
			<h3 class="text-primary text_orange font-weight-bold m-0 line-height-'normal mt-2 text-center mb-3" style="font-size:18px;">COMPETITIVE METRICS</h3><input type="hidden" value="<%=session("EID")%>" id="txtsession">
				...
				<div class="w-100">
						<div class="chosen-wrapper" data-js="custom-scroll">
							...
							<select class="chosen-select" id="region_selection_dropdown_01" data-placeholder="WORLD RANK & SHARE">
										...
										<th>
											<div class="d-flex">
												<div class="col-02">
													...
													<span title="Strong">S</span>
													<span title="Moderate">M</span>
													<span title="Niche">N</span>
													<span title="Trivial">T</span>
				...
				'	response.end
					...
					If not RSInfluncer.EOF Then 
						...
						Set rsMetrics = Server.CreateObject("ADODB.Recordset")
							if trim(session("EID")) <>"" then
								SqlMetrics="select * from tbl_Competitive_metrics where CP_code in ('"&RSInfluncer("Filename")&"') and MCP_code='" & code & "' and (email='" & session("eid") & "')"
							else
								SqlMetrics="select * from tbl_Competitive_metrics where CP_code in ('"&RSInfluncer("Filename")&"') and MCP_code='" & code & "' and (Session_id='" & session.SessionID & "')"
							end if	
						...
						<!---ROW BEGIN (THIS ROW TO BE REPEATED) -->
											...
											<tr>
                                                <td>
													...
													<%else%>
														<h4 title="<%=RSInfluncer("company")%>"> <a href="https://www.strategyr.com/market-share-rank.asp?code=<%=RSInfluncer("Filename_MD5")%>&pcode=<%=code%>" class="text-black" target="_blank"> <%=RSInfluncer("company")%></a> </h4>
													<%end if%>
                                                        ...
                                                        <div class="col-03">
															...
															<%if session("EID") ="" then%>  <!--<p class="input_popup_msg"> Login Required</p>--><%end if%>
                                                                    ...
                                                                    <div class="range-slider simulator_block">
                                                                        <input type="text"  onchange="AddComp_metris('<%=RSInfluncer("filename")%>','<%=code%>')" value="<%=Rank%>" class="js-range-slider"
                                ...
                                <td class="comp_sub">
									<p class="font-weight-bold mb-0">                                         
                                         <div class="float-left d-flex justify-content-start flex-column">
											<div><span class="text-danger">*</span> S = Strong; M = Moderate; N = Niche; T= Trivial (<1%)</div>
											<div>
											...
											<i><span class="text-danger">*</span>Participating executives must be from a competitive company. Executive credentials are validated by a one-time two-factor authentication and fully compliant with GDPR guidelines. Verified executives have complimentary access to our proprietary platform's data silos including Trend Data; Brand/IP/TM Feeds; and Market Size, Share & Rank Clusters. Qualified influencers are also invited to our Expert Panels enabling peer-to-peer collaborative bespoke builds, and will receive special product offerings.</i>											 
											</div>
                                            <%if session("EID") <>"" then%>
												<a  href="Report_CompetitiveMetrics.asp?code=<%=code%>" target="new" class="font-size-15 font-weight-bold text-left ml-1">View All<%'=companies%> Companies...</a> 
											<%else%>											
												<a  href="LoginRepeat.asp?login=compMetrics" class="font-size-15 font-weight-bold text-left ml-1"  >View All<%'=companies%> Companies...</a> 
											<%end if%>
											
                                         </div>
                                     </p>
									<%if trim(session("EID"))<>"" then%>
										<a href="#" onclick="AddAllComp_metris('<%=code%>')" class="submit-button-200 my-2  float-right" type="button" > PARTICIPATE NOW! </a>
									<%else%>
