...
================================================== -->
	...
	<%',res_brand_image_url
						...
						<div class="w-lg-30 w-xl-30 w-md-45 w-sm-100  d-flex flex-column shadow  p-2 border my-2">				
							<div class="w-100 ">
                                <div id="label_container" class="bg-white cards m-0 p-0 ">
								<div class="top_block mt-2 ">
										<h4 class="font-weight-bold  mb-1 pb-1  text-center border-bottom" style="color:#900"> <%=RSwBrands("res_brand_name")%></h4>
								</div>
                                    <div class="label_middle_block d-flex text-center">
                                        <div class="  d-flex flex-row  flex-md-row position-relative align-content-center w-100 ">
											...
											%>
                                            <div class="text-left   label_aside mt-sm-0 mt-md-2 mt-lg-2 mt-xl-2" style="margin-left:4px">
                                                
                                               <div class="d-flex mt-2">
											   <h4 class="mb-0 d-flex  columns px-0 mr-3 ml-0 "><input type="radio" name="star_rating<%=RSwBrands("res_brand_name")%>" id="<%=RSwBrands("res_brand_name")%>Dominant" value="Dominant"  <%=SelectRadio(CompetitiveChkedValue,"Dominant")%> onClick="Addinfluence_moduletoDB('<%=RSwBrands("sno")%>','<%=Trim(request("code"))%>','<%=Trim(request("encID"))%>','Dominant','<%=RSwBrands("company_cpcode")%>','<%=RSwBrands("res_brand_name")%>')">Dominant</h4><br>
                                                <h4 class="mb-0 d-flex  columns px-0 mr-3 ml-0  "><input type="radio" name="star_rating<%=RSwBrands("res_brand_name")%>" id="<%=RSwBrands("res_brand_name")%>Active" value="Active" <%=SelectRadio(CompetitiveChkedValue,"Active")%> onClick="Addinfluence_moduletoDB('<%=RSwBrands("sno")%>','<%=Trim(request("code"))%>','<%=Trim(request("encID"))%>','Active','<%=RSwBrands("company_cpcode")%>','<%=RSwBrands("res_brand_name")%>')">Active</h4><br>
												</div>
												 <div class="d-flex mt-2">
												<h4 class="mb-0 d-flex  columns px-0 mr-3 ml-0 "><input type="radio" name="star_rating<%=RSwBrands("res_brand_name")%>" id="<%=RSwBrands("res_brand_name")%>Niche" value="Niche"  <%=SelectRadio(CompetitiveChkedValue,"Niche")%> onClick="Addinfluence_moduletoDB('<%=RSwBrands("sno")%>','<%=Trim(request("code"))%>','<%=Trim(request("encID"))%>','Niche','<%=RSwBrands("company_cpcode")%>','<%=RSwBrands("res_brand_name")%>')">Niche</h4><br>
												<h4 class="mb-1 d-flex  columns pl-2 mr-3 " style="margin-left: 20px !important;"><input type="radio" name="star_rating<%=RSwBrands("res_brand_name")%>" id="<%=RSwBrands("res_brand_name")%>N/A" <%=SelectRadio(CompetitiveChkedValue,"N/A")%>  value="N/A" onClick="Addinfluence_moduletoDB('<%=RSwBrands("sno")%>','<%=Trim(request("code"))%>','<%=Trim(request("encID"))%>','N/A','<%=RSwBrands("company_cpcode")%>','<%=RSwBrands("res_brand_name")%>')">N/A</h4>
												</div>
                                            </div>
                                        </div>
